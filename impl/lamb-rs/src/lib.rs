...
pub unsafe extern "C" fn pretty_print(x: *mut ffi::AstNode_T) {
    let script = ffi::Script::from_ptr(x);
    println!("{script:#?}");
}
...
/// which respects the structure generated by the current Flex / Bison definition.
#[no_mangle]
pub unsafe extern "C" fn optimize(node: *mut ffi::AstNode_T) -> *mut ffi::AstNode_T {
    if node.is_null() {
        return node;
    }

    match ffi::Script::from_ptr(node) {
        Ok(mut script) => {
