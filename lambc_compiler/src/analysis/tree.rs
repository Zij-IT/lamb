...
use crate::{
    name_res::Var,
    type_check::{Type, TypeScheme, TypedVar},
    PathRef,
};
...
pub struct Module {
    ...
    /// The top-level definitions within a file
    pub defs: Vec<TopDefine>,
    ...
    pub fn is_rest_pattern(&self) -> bool {
        match self {
            InnerPattern::Ident(i) => {
                i.bound.as_deref().map_or(false, InnerPattern::is_rest_pattern)
            }
...
pub struct ArrayPattern {
    /// the patterns which appear before the rest pattern (or all if there is no rest)
    pub head: Vec<Pattern>,
    /// the rest pattern and the patterns the follow
    pub rest: Option<(RestPattern, Vec<Pattern>)>,
    /// The span of the entire pattern
    pub span: Span,
}
